...
class cs4wbase_t {
    ...
    */
    ...
    std::vector<CounterType, Allocator<CounterType>> core_;
    uint32_t np_, nh_; // nh_ hsse number of hashes
    uint64_t mask_;
    uint64_t seedseed_;
...
public:
    ...
    CounterType addh_val(uint64_t val) {
        std::vector<CounterType> counts(nh_); // counter type vector nisse
        auto cptr = counts.data();
        for(unsigned added = 0; added < nh_; ++added) //no of hash songhok bar loop
            cptr[added] = add(val, added); // val, added maybe hash number
        return median(cptr, nh_); // why median?
    }
    auto addh(uint64_t val) {return addh_val(val);} //jst addh_val return kre
    auto nhashes() const {return nh_;} //hash songkha return kre
    ...
    INLINE auto add(uint64_t hv, unsigned subidx) noexcept {
        hv = hf_(hv, subidx); //value and hash number diye hash index passe
    ...
    INLINE int sign(uint64_t hv) const noexcept {
        return hv & (1ul << np_) ? 1: -1; //hv pos or neg check
    }
    using Space = vec::SIMDTypes<uint64_t>;
    INLINE void subh(Space::VType hv) noexcept {
        hv.for_each([&](auto x) {for(size_t i = 0; i < nh_; sub(x, i++));}); //onekgula eksathe sub
    }
    INLINE void addh(Space::VType hv) noexcept {
        hv.for_each([&](auto x) {for(size_t i = 0; i < nh_; add(x, i++));}); //onekgula eksathe add
    }
    CounterType est_count(uint64_t val) const {
        common::detail::tmpbuffer<CounterType> mem(nh_);
        CounterType *ptr = mem.get(), *p = ptr;
        for(unsigned i = 0; i < nh_; ++i) { //loop over number of hash
            auto v = hf_(val, i); //maybe hash er index
            *p++ = at_pos(v, i) * sign(v);
        }
        return median(ptr, nh_); //median kno?
    }
