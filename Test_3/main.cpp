        ...
        if( line.empty() || line[0] == '>' ){
            if( !name.empty() ){
                ...
                for(int i=0; i<=l-len_k_mer; i++) //extracting k-mers from the line
                {
                    ...
                    /////////Updating part.
                    ccmbase_obj_name.addh_val(k_mer);
...
int main(){
    ...
    sketch::ocm::ccmbase <int32_t, sketch::hash::HasherSet<sketch::hash::WangHash> > cms_obj(19,100,137,false);   // np = 5, nh = 10, seed = 137,
    sketch::ocm::ccmbase <int32_t, sketch::hash::HasherSet<sketch::hash::WangHash> > ccms_obj(19,100,137,true);
    ...
    //construct OCM
    sketch::ocm::ocmbase <uint64_t, sketch::hash::HasherSet<sketch::hash::WangHash> > sketch1(19,100,137);
    for(int r = 1; r<= 3; r++){
        if (r > 1){
